...
const AddService = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!formData.vendor || !formData.status) {
        alert("Mohon lengkapi Vendor dan Status!");
        return;
    }
  ...
  if (loading) return <MainLayout><div className="h-screen flex items-center justify-center"><Loader2 className="animate-spin mr-2"/> Memuat Form...</div></MainLayout>;
      ...
      <div className="max-w-4xl mx-auto space-y-6 pb-20">
        ...
        <form onSubmit={handleSubmit} className="bg-white p-8 rounded-xl shadow-sm border border-gray-100 grid grid-cols-1 md:grid-cols-2 gap-6">
          <div>
            ...
            <input type="text" name="ticket_no" placeholder="Contoh: SIM-SJI..." value={formData.ticket_no} onChange={handleChange} className="w-full px-3 py-2 border rounded-lg outline-none focus:ring-2 focus:ring-penabur-blue"/>
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">Tanggal Service</label>
            <input type="date" name="service_date" value={formData.service_date} onChange={handleChange} className="w-full px-3 py-2 border rounded-lg outline-none focus:ring-2 focus:ring-penabur-blue"/>
          </div>
          ...
          <div className="md:col-span-2">
            ...
            <input type="text" required maxLength="50" name="issue_description" placeholder="Deskripsi Singkat Kerusakan" value={formData.issue_description} onChange={handleChange} className="w-full px-3 py-2 border rounded-lg outline-none"/>
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">Vendor <span className="text-red-500">*</span></label>
            <select required name="vendor" value={formData.vendor} onChange={handleChange} className="w-full px-3 py-2 border rounded-lg outline-none focus:ring-2 focus:ring-penabur-blue bg-white">
          ...
          <div>
            ...
            <select required name="status" value={formData.status} onChange={handleChange} className="w-full px-3 py-2 border rounded-lg outline-none font-bold text-gray-700 bg-white">
          ...
          <div className="md:col-span-2 flex justify-end pt-4">
             <button type="submit" disabled={isSubmitting} className="bg-penabur-blue text-white px-8 py-3 rounded-lg font-bold hover:bg-penabur-dark flex items-center shadow-lg transform hover:-translate-y-1 transition-all disabled:opacity-70">
